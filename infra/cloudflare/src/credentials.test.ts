...
it.effect("accepts an owner-only file that declares every deployment input", () =>
  Effect.gen(function* program() {
    ...
    assert.strictEqual(yield* readOwnerOnlySecretsFile(filename), filename);
  }).pipe(Effect.scoped),
);
...
it.effect("names the inputs the file is missing", () =>
  Effect.gen(function* program() {
    ...
    const failure = yield* readOwnerOnlySecretsFile(filename).pipe(Effect.flip);
    assert.strictEqual(failure.code, "secrets_file_incomplete");
    assert.deepStrictEqual(
      failure.keys,
      settingsKeys.filter((key) => key !== "CLOUDFLARE_API_TOKEN"),
    );
  }).pipe(Effect.scoped),
);
...
it.effect("refuses a file reached through a symbolic link", () =>
  Effect.gen(function* program() {
    ...
    const failure = yield* readOwnerOnlySecretsFile(link).pipe(Effect.flip);
...
it.effect("refuses a directory reached through a symbolic link", () =>
  Effect.gen(function* program() {
    ...
    yield* Effect.promise(async () => symlink(real, link));
    const failure = yield* readOwnerOnlySecretsFile(path.join(link, "cloudflare.env")).pipe(
      Effect.flip,
    );
    assert.strictEqual(failure.code, "secrets_file_symlink_forbidden");
  }).pipe(Effect.scoped),
...
it.effect("reports a missing file instead of deploying without it", () =>
  Effect.gen(function* program() {
    const directory = yield* temporaryDirectory();
    const failure = yield* readOwnerOnlySecretsFile(path.join(directory, "cloudflare.env")).pipe(
      Effect.flip,
    );
...
it.effect("the deployment inputs live outside the repository and stay overridable", () =>
