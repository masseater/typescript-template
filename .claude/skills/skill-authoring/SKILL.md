...
- 言い換えるときは元の語の限定を落とさない（「実測」を「計測」にしない）。擬人化は別の擬人化に置き換えず、「警告なしに」「エラーを出さずに」のように何が起きないかで書く。
- textlint の AI 語の指摘は検討の合図であり、置き換えの命令ではない。言い換えると意味が変わる語は `.textlintrc.json` の `allows` に足し、定着させたくない言い換えは `.textlint-ai-words.json` に足す。
