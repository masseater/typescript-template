...
既存の skill や AGENTS.md を削るときは、`references/pruning.md` の手順で、実行して結果が変わらない文だけを消す。

指示なしで頼むとこのリポジトリの規則に反する形が出てくる作業には、具体例を書く。例には正解の形だけを書き、避けたい形は並べない。
...
- textlint の AI 語の指摘には、意味を保つ英語や定着したカタカナ語で応える。意味の弱い和語に逃げない。`tools/dont-review-it/src/features/dont-review-it/repository/text-lint.ts` のルール設定の `allows` にはユーザーの指示なしに語を足さない。定着させたくない言い換えは `.textlint-ai-words.json` に足す。
