...
- ワークスペース定義の無いリポジトリでは依存を確認しない
- 解釈できないワークスペース定義を、どの確認も通り抜ける前に報告する
...
- すべてを採ると、どの束のルールも入る
- 名指しした束のルールだけが入る
- 報告を消す手段を塞ぐ束は、名指ししなくても入る
- 報告を消す手段を塞ぐ束だけを名指ししても、二重には入らない
- 採っていない束の確認は、実行せずに理由を添えて並べる
- 束を名指ししていないツールチェーン設定では、どの確認も動く
...
- すべてのワークスペースが preset の下にある設定を何も言わずに通す
...
- ツールチェーンの設定が無いリポジトリでは適用範囲を確認しない
...
- 解釈できない定義を、どの確認も通り抜ける前に場所を指して報告する
