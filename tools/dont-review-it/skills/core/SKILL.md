...
name: core
description: >
  Adopt @template/dont-review-it in a Vite+ repository: call `dontReviewItPreset.fmt()` and `dontReviewItPreset.lint()` inside the `fmt` and `lint` blocks of `vite.config.ts`. The preset rejects `function` declarations, `export default`, `interface`, truthiness tests, type assertions, reassignment and in-place mutation, `.then` chains, `console`, explanatory comments, `beforeEach`, and every form of rule suppression, and caps `max-params` at 2 and `max-statements` at 10. Extend `@template/dont-review-it/tsconfig/app.json` or `library.json` from every tsconfig, capture process output with `standardIoTest` from `@template/dont-review-it/vitest`, and prove a custom rule is wired with a violating probe file. Load when configuring oxlint or oxfmt with this preset, when a `dont-review-it/*` rule reports, when you need to know what the preset actually rejects, or when `ignorePatterns` or `lint.plugins` seem to have no effect.
metadata:
  type: core
  library: "@template/dont-review-it"
...
# @template/dont-review-it — adopt the preset
...
Each function returns the whole object its block must own: the patterns git is told to ignore, and — for `lint` — the rule sets of `@template/lint-rule-authoring` and `@template/dont-review-it` together with the JS plugin that holds the custom rules, or — for `fmt` — the formatting choices this repository fixes. Whatever you pass in layers on top: `lint({ rules: { ... } })` keeps your rules, and an `extends` you pass lands after the shipped presets.
...
```json
{
  "extends": "@template/dont-review-it/tsconfig/library.json",
  "include": ["src"]
}
```
...
```
@template/dont-review-it                    dontReviewItPreset { fmt, lint }
@template/dont-review-it/plugin             the oxlint jsPlugins entry (registered by the preset)
@template/dont-review-it/vitest             standardIoTest, CapturedStream
@template/dont-review-it/tsconfig/app.json      application workspaces
@template/dont-review-it/tsconfig/library.json  published packages
