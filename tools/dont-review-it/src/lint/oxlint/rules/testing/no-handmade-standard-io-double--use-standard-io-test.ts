...
export const noHandmadeStandardIoDouble = createDontReviewItRule({
  ...
  meta: {
    ...
    messages: {
      ownFixture:
        "A spec must not declare a `{{name}}` fixture of its own. Import `standardIoTest` from `@template/dont-review-it/vitest` and derive the test from it.",
      directStream:
        "A spec must not reach `process.{{name}}` by hand. Import `standardIoTest` from `@template/dont-review-it/vitest`; its `{{name}}` fixture hands the captured stream to the test.",
      streamShapedDouble:
        "A spec must not assemble a `{{name}}`-shaped write double. Import `standardIoTest` from `@template/dont-review-it/vitest` and assert on its `{{name}}` fixture instead.",
    },
    schema: [],
  },
  create(inspection) {
