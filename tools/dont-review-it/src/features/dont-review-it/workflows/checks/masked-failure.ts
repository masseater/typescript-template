import { entriesNamedInJobsAndSteps, entriesNamedInSteps } from "../steps.ts";
import { isTruthyScalar, lineOf, scalarText, type WorkflowDocument } from "../workflow-document.ts";
...
export const maskedFailures = ({
  ...
  ...entriesNamedInJobsAndSteps({ document, config, key: config.continueOnErrorKey })
    .filter((listed) => isTruthyScalar(listed.value))
    .map((listed) => ({
