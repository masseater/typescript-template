...
const databaseOperations = String.raw`^libs/db/src/features/db/(?:remote|bootstrap|migrat)[^/]*\.ts$`;
const databaseInternal = String.raw`^libs/db/src/features/db/(?:(?:remote|bootstrap|migrat|testing)[^/]*\.ts$|.*${testModule})`;
...
const objectStorageOwner = String.raw`^libs/(?:config|runtime|vite-config)/src/|^infra/cloudflare/src/features/cloudflare/`;
...
const configuration = {
  forbidden: [
    ...
    },
    {
      ...
      from: {
        path: "^(?:apps|libs)/",
        pathNot: `^apps/service-admin/|^libs/db/src/features/db/(?!index\\.ts$)|${testModule}`,
      },
