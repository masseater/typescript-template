...
const observabilityAnnotations = "/libs/observability/src/features/observability/annotations.ts";
const observabilitySeverity = "/libs/observability/src/features/observability/severity.ts";
...
const crossRequestStateVisitor = (inspection: LintContext): Visitor => {
  ...
  if (
    ...
    inspected.endsWith("/libs/runtime/src/features/runtime/worker-runtime.ts")
  ) {
    return {};
  }
...
const projectPlugin = definePlugin({
  ...
  rules: {
    ...
    boundaries: {
      ...
      meta: metadata(
        `依存境界違反です。配布物に入るコードの依存先は、文字列リテラルだけで指定してください。連結・テンプレート・変数の経由と require・createRequire は、依存グラフの検査が追えないので使えません。パッケージ間の向きは dependency-cruiser が tools/dont-review-it/src/features/dont-review-it/repository/dependency-cruiser.ts の規則で判定します。生 D1 操作は ${rawD1Modules.join(" と ")} だけに限定し、業務処理は計測付き ORM を使用してください。`,
      ),
    },
    "cross-request-state": {
    ...
    "effect-stack": {
      ...
      meta: metadata(
        "入力検証は valibot ではなく effect の Schema で行ってください。Elysia アプリは libs/runtime/src/features/runtime/http.ts の createApi で作り、createFileRoute の server には elysiaServer(app) だけを渡して Elysia に委譲してください。handlers を直接書いた server route は作れません。",
      ),
    },
    ...
    "modular-layers": {
      create: modularLayersVisitor,
      meta: metadata(
        "Modular パッケージでは、src の直下に置けるのは app・features・shared だけです。業務の単位は features/<name> に置き、横断だけを shared、入口の配線だけを app に残してください。",
      ),
    },
    "modular-imports": {
