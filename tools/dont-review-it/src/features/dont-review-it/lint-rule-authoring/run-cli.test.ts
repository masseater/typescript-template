...
const MISSING_INDEX = `packages/example/docs/lint/index.md A workspace that declares lint rules must not go without \`packages/example/docs/lint/index.md\`. Generate it with \`dont-review-it regenerate\`.\n`;

const MISSING_DOC = `packages/example/docs/lint/no-thing--allow-it.md A rule must not go without its document. Seed it with \`dont-review-it regenerate\`, then write the sections it leaves for you.\n`;

const MISSING_GUIDELINE_INDEX = `docs/lint-rules-by-guideline.md A repository whose rules name their grounds must not go without \`docs/lint-rules-by-guideline.md\`. Generate it with \`dont-review-it regenerate\`.\n`;
