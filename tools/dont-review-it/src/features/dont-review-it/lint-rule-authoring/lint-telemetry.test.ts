...
describe("startLintTelemetry", () => {
  describe("an environment that never asked for telemetry", () => {
    const it = test.extend("startAnswer", () =>
      Effect.runPromise(
        Effect.gen(function* startAnswer() {
  ...
  describe("an environment that asked for telemetry and started once already", () => {
    const it = test.extend("restartAnswer", () =>
      Effect.runPromise(
        Effect.gen(function* restartAnswer() {
          vi.stubEnv("MST_TELEMETRY", "1");
          vi.stubEnv("OTEL_SDK_DISABLED", undefined);
          onTestFinished(() => {
            process.exitCode = undefined;
  ...
  describe("a process winding down after a rule duration was recorded", () => {
    const it = test.extend("windDownExports", () =>
      Effect.runPromise(
        Effect.gen(function* windDownExports() {
          vi.stubEnv("MST_TELEMETRY", "1");
          vi.stubEnv("OTEL_SDK_DISABLED", undefined);
          onTestFinished(() => {
            process.exitCode = undefined;
  ...
  describe("an export that fails", () => {
    describe("the exit code the process carried when the failure was reported", () => {
      const it = test.extend("exitCodeCarriedIntoTheReport", () =>
        Effect.runPromise(
          Effect.gen(function* exitCodeCarriedIntoTheReport() {
            vi.stubEnv("MST_TELEMETRY", "1");
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
    ...
    describe("a failure carrying an error", () => {
      const it = test.extend("thrownErrorReport", () =>
        Effect.runPromise(
          Effect.gen(function* thrownErrorReport() {
            vi.stubEnv("MST_TELEMETRY", "1");
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
    ...
    describe("a failure carrying a value that is not an error", () => {
      const it = test.extend("thrownNonErrorReport", () =>
        Effect.runPromise(
          Effect.gen(function* thrownNonErrorReport() {
            vi.stubEnv("MST_TELEMETRY", "1");
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
...
describe("measureStage", () => {
  describe("a stage measured in an environment that never asked for telemetry", () => {
    describe("what the measured stage produced", () => {
      const it = test.extend("stageProductWithoutDurations", () =>
        Effect.runPromise(
          Effect.gen(function* stageProductWithoutDurations() {
            vi.stubEnv("MST_TELEMETRY", undefined);
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
    ...
    describe("the exporter behind a stage nobody asked to measure", () => {
      const it = test.extend("exportsWithoutDurations", () =>
        Effect.runPromise(
          Effect.gen(function* exportsWithoutDurations() {
            vi.stubEnv("MST_TELEMETRY", undefined);
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
  ...
  describe("a stage measured in an environment that asked for telemetry", () => {
    describe("what the measured stage produced", () => {
      const it = test.extend("stageProductWithDurations", () =>
        Effect.runPromise(
          Effect.gen(function* stageProductWithDurations() {
            vi.stubEnv("MST_TELEMETRY", "1");
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
    ...
    describe("the metrics the exporter was handed as the process wound down", () => {
      const it = test.extend("windDownStageMetricNames", () =>
        Effect.runPromise(
          Effect.gen(function* windDownStageMetricNames() {
            vi.stubEnv("MST_TELEMETRY", "1");
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
  ...
  describe("a stage measured while another stage is already being measured", () => {
    const it = test.extend("nestedStageNames", () =>
      Effect.runPromise(
        Effect.gen(function* nestedStageNames() {
          vi.stubEnv("MST_TELEMETRY", "1");
          vi.stubEnv("OTEL_SDK_DISABLED", undefined);
          onTestFinished(() => {
            process.exitCode = undefined;
                  ...
                  scope.metrics.flatMap((metric) =>
                    metric.dataPointType === DataPointType.HISTOGRAM
                      ? metric.dataPoints.flatMap((point) =>
                          typeof point.attributes.stage === "string"
...
describe("ruleDuration", () => {
  describe("the histogram behind a second call", () => {
    const it = test
      .extend("startedTelemetry", () =>
        Effect.runPromise(
          Effect.gen(function* startedTelemetry() {
            vi.stubEnv("MST_TELEMETRY", "1");
            vi.stubEnv("OTEL_SDK_DISABLED", undefined);
            onTestFinished(() => {
              process.exitCode = undefined;
