...
const MISSING = `A repository whose rules name their grounds must not go without \`${INDEX_PATH}\`. Generate it with \`dont-review-it regenerate\`.`;
...
const STALE = `\`${INDEX_PATH}\` must not fall behind the grounds its rules declare. Regenerate it with \`dont-review-it regenerate\`.`;
