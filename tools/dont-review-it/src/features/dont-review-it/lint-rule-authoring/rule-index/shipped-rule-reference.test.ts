...
const MISSING_REFERENCE = `A package that ships both lint rules and agent skills must not go without \`${REFERENCE_PATH}\`, because the rule documents stay in the repository and never reach an installed copy. Generate it with \`dont-review-it regenerate\`.`;

const MISSING_MARKERS = `\`${REFERENCE_PATH}\` must not lose its generated region. Put \`<!-- BEGIN GENERATED shipped-lint-rules -->\` and \`<!-- END GENERATED shipped-lint-rules -->\` back, or delete the file and regenerate it with \`dont-review-it regenerate\`.`;

const STALE_REFERENCE = `\`${REFERENCE_PATH}\` must not fall behind the rule implementations it lists. Regenerate it with \`dont-review-it regenerate\`.`;
...
const WRITTEN_REFERENCE = [
  ...
  "",
  "Every rule below is registered at error severity unless the table says the preset leaves it off. Generated from the rule implementations; regenerate with `dont-review-it regenerate` rather than editing it.",
  "",
  "<!-- BEGIN GENERATED shipped-lint-rules -->",
  "",
