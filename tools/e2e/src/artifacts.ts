...
const privateWikiSource =
  /^(?:apps\/(?:user|admin)\/|libs\/db\/src\/(?:admin|remote[^/]*|bootstrap[^/]*|testing)\.ts$)/;
...
export function assertSeparation(pair: ArtifactPair): void {
  ...
  for (const marker of adminMarkers) {
    ...
    pair.admin.sources.server.includes("apps/admin/src/routes/api.users.ts"),
