...
import { ensure, fail } from "./support.ts";

export const totp = Effect.fn("totp")(function* (uri: string, now = Date.now()) {
  const parsed = URL.parse(uri);
  if (parsed?.protocol !== "otpauth:" || parsed.hostname !== "totp")
    return yield* fail("E2E_INVALID_TOTP_URI");
  const encoded = parsed.searchParams.get("secret")?.toUpperCase().replace(/=+$/, "");
  if (!encoded || !/^[A-Z2-7]+$/.test(encoded)) return yield* fail("E2E_INVALID_TOTP_SECRET");
