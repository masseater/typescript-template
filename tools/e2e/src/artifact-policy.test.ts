...
function data(): ArtifactPair {
  return {
    ...
    admin: {
      ...
      sources: {
        server: ["apps/admin/src/routes/api.users.ts", "libs/db/src/admin.ts"],
...
test("the wiki bundle is a separate Worker with its own login but without application code", () => {
  ...
  const loginSources = sourcePaths(
    new Map([
      [
        "auth.js.map",
  ...
  expect(() =>
    assertSeparation({
      ...
      wiki: {
        ...
        server: login,
        sources: {
          server: [...pair.wiki.sources.server, ...loginSources],
          client: [...pair.wiki.sources.client, "libs/ui/src/auth.tsx"],
        },
      },
    }),
  ).not.toThrow();
  ...
  for (const source of ["libs/db/src/admin.ts", "apps/admin/src/routes/api.users.ts"]) {
    expect(() =>
      assertSeparation({
        ...pair,
        wiki: {
          ...pair.wiki,
          sources: { ...pair.wiki.sources, server: [...pair.wiki.sources.server, source] },
        },
      }),
    ...
    expect(() =>
      assertSeparation({
        ...pair,
        wiki: { ...pair.wiki, sources: { ...pair.wiki.sources, client: [source] } },
      }),
