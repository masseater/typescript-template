...
function callResult(reference: D1Reference, bound: boolean): D1Reference[] {
  if (bound) {
    return [reference];
  }
...
function markDynamic(reference: D1Reference): D1Reference {
...
function workerBindingReferences(
...
function assignmentPattern(node: Node): Node {
  const { parent } = node;
