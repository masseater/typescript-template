...
const rootTsconfig: Readonly<Record<string, { readonly default?: unknown }>> = import.meta.glob(
  "../../tsconfig.json",
  { eager: true },
);
...
const rootIncludesDoctor = (): boolean => {
  const [module] = Object.values(rootTsconfig);
  const include = includeValue(module?.default);
  return include?.includes("doctor.config.ts") ?? false;
};
...
describe("inspection coverage", () => {
  ...
  it("typechecks workspace vite configs and the root doctor config", () => {
    expect.hasAssertions();
    expect(viteConfigsOutsideInclude()).toStrictEqual([]);
    expect(rootIncludesDoctor()).toBe(true);
  });
