...
const QueryInput = Schema.Struct({
  command: Schema.Literals(["logs", "traces", "trace", "request"]),
...
const query = Effect.gen(function* () {
  ...
  if (input.command === "request") {
    if (input.requestId === undefined) return yield* argumentsInvalid();
    return { command: input.command, data: yield* requestTelemetry(values.app, input.requestId) };
  }
  if (input.command === "trace") {
    if (input.traceId === undefined) return yield* argumentsInvalid();
    return {
      command: input.command,
      data: yield* queryExplorer(
        values.app,
