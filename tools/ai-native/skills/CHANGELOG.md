...
- `@template/ai-native/telemetry`, the one place a process starts its OpenTelemetry provider. The first caller fixes `service.name`, `MST_TELEMETRY` turns measurement on, `OTEL_SDK_DISABLED` turns it back off, and shutdown runs after every other `beforeExit` handler.
- `@template/ai-native/vitest-sdk`, the entry Vitest's `experimental.openTelemetry.sdkPath` is pointed at.
