...
- 画面の見た目・UI コンポーネント・レイアウトを触るときは、先にルートの `DESIGN.md` を読む。トークンの SSOT は `@repo/ui` の stylesheet で、`DESIGN.md` の YAML はそれとテストで同期する。
...
- main への merge は、PR のセルフレビューが終わった時点で draft を外し、`ready-to-merge` ラベルを付けて Mergify の merge queue に積む。CI の成功も、先に積んだ PR の merge も待たない。一件ずつ空くのを待たず、積めるものは積む。ベース更新で PR の CI が止まると、成功待ちのラベル付与はキューに入らないまま止まる。CI が落ちたら直して push し、ラベルは付けたままにする。
- draft 解除と queue 投入はエージェント自身の判断で行うこと。人間に undraft やラベル付与を任せない。親 PR 待ちの stacked PR も、セルフレビュー済みなら `Depends-On` を付けたうえで自ら undraft し `ready-to-merge` を付ける。
- コードの簡潔さや様式より、機械による問題検出と、実アプリの直接操作で計測したデータの直接照会を優先する。
...
- ドキュメント・コードコメントは書かない。ドキュメントもコードも、コードを読めば分かるので、生まれた時点で矛盾を生み出す要因となるため。何故これをする必要があるのか、という経緯はコミットログに書くこと。
