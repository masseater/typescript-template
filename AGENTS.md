...
- 画面の見た目・UI 部品・レイアウトを触るときは、先にルートの `DESIGN.md` を読む。トークンの正本は `@repo/ui` の stylesheet で、`DESIGN.md` の YAML はそれとテストで同期する。
...
- セッションリプレイ（利用者の操作の録画と再生）は統合しない。録画の保存と検索の費用が他の信号より桁で大きいため。エラーの再現に操作の再現が要るようになったときにだけ検討し、その場合もエラーが起きたセッションだけを録画する構成から始める。
