...
- main への merge は、PR のセルフレビューが終わった時点で `ready-to-merge` ラベルを付けて Mergify の merge queue に入れる。CI の成功を待ってからラベルを付けない。ベース更新で PR の CI が止まると、成功待ちのラベル付与はキューに入らないまま止まる。CI が落ちたら直して push し、ラベルは付けたままにする。
