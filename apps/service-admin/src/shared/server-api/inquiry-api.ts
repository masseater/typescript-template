...
function inquiryApi<Requirements = never>(api: ApiRoutes<AppServices | Requirements>) {
  return createApi("")
    ...
    .get("/inquiries/pending-count", ...api.route({ response: PendingCount }, pendingCount, failures))
    .get("/inquiries/detail", ...api.route({ response: AdminInquiryThread }, inquiryDetail, failures))
    .get("/inquiries/member", ...api.route({ response: InquiryMemberSummary }, memberSummary, failures))
