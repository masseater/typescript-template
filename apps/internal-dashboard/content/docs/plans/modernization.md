...
| ★3 | セッションリプレイ | 利用者の操作を DOM の変化として録画し、trace id で Workers のトレースと結びつけます | 採用しない。理由: 録画は 1 セッションで数百 KB から数 MB になり、保存と検索の費用が他の信号を上回ります |
...
セッションリプレイは採用しません。費用が主な理由です。

利用者の操作を再生できると、エラーやトレースが指す時点で利用者が何を見ていたかが分かります。ただし録画はログやスパンと違って 1 セッションごとに DOM の全変化を持つため、保存量が他の信号より 1 桁から 2 桁大きく、事業の規模が決まる前に保存と検索の費用だけが先に立ちます。SaaS の無料枠に載せる場合も、録画数の上限がそのまま観測できる範囲の上限になります。

後から入れるときに変更が要らない部分は既にあります。`@repo/observability` の OTLP 出力は宛先と認証ヘッダーをデプロイ入力で切り替えられ、`@repo/ui` のブラウザ計測は `traceparent` を要求に載せ、Workers 側の要求スパンがそれを親として継承します。録画のセッション識別子をこの要求へ載せる部分だけが足りていません。

入れる判断をするのは、エラーの再現に利用者の操作の再現が要るようになったときです。そのときは全セッションを録画せず、エラーが起きたセッションだけを録画する設定から始めます。
