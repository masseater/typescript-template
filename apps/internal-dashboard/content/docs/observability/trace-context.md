...
`00` は版、次の 32 桁が trace id、次の 16 桁が親の span id、末尾の `01` は sampled である。この要求の処理を span にする側は、応答の `traceparent` に同じ trace id と自分の span id を載せる。`tracestate` は同じ要求に付く別ヘッダで、ベンダが自分のキーを載せる。trace id は `traceparent` が持ち、`tracestate` はそれを置き換えない。別のキーを同じ要求に載せるヘッダは `baggage` で、trace id とは別である。span を開いてこのヘッダを読む処理の形は [OpenTelemetry SDK](/observability/opentelemetry-sdk) が持ち、このテンプレートでの読み方は [OpenTelemetry](/tech-stack/opentelemetry) が持つ。
