...
HTTP は、ブラウザとサーバが要求と応答を往復させるプロトコルである。状態コードは応答の意味を一つに決め、所要時間は送り始めから応答を受け終わるまでである。どの状態コードを選ぶかは `.claude/skills/reviews/references/monitoring-logging-and-status-codes.md` が持つ。
