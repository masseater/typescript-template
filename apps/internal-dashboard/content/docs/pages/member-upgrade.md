...
3. 料金の表示（見本の文言でよく、通貨と周期を 1 行で出す）
4. 「契約する」ボタン
5. 「いまは無料のまま使う」リンク（ひとつ前のページ、無ければホームへ戻る）

- 引き止めや、無料のままでは足りないことを長く並べるブロックは置かない
- 「契約する」の先は、支払い事業者の手続きへ移る。成功したら元の操作のページへ戻し、失敗したらこのページに理由を出す
...
flowchart TD
  ...
  upgrade -- 契約に成功 --> back["元の操作のページ"]
  upgrade -- いまは無料のまま --> back
