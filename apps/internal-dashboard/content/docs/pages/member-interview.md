...
- `/welcome/interview`: [登録の枠](/pages/member-layout#登録の枠) に入り、登録の直後にプロフィールを作るときに使う
- `/settings/interview`: [設定](/pages/member-layout#設定) に入り、登録を終えた後にやり直すときに使う
...
- 保存のあと、会話の履歴を残して次からのレコメンドに使ってよいかを確かめる
  - 「残す」と「残さない」を置く
  - 残すを選んだときだけ、履歴を保ち、[規約への同意](/pages/member-agreement) と同じ仕組みで同意を記録する
  - 残さないを選んだときは、保存したシートだけを残し、会話の履歴は消す
  - すでに最新版へ同意済みのときは、この確認を出さず保存済みへ進む
...
| 履歴の同意を求めている | 保存のあと、履歴を残すかどうかの確認を出す |
...
flowchart TD
  ...
  summary -- この内容で保存 --> consent["履歴の同意"]
  consent -- 残す --> saved["保存済み"]
  consent -- 残さない --> saved
  saved -- 登録の枠 --> home
  saved -- 文章で直す --> summary
