...
| Wiki の編集 | `/wiki-edit/{文書のパス}` | 出さない | 見る・下書きの保存と破棄 | 見る・下書きの保存と破棄・公開 |
...
文書を書き換えるページで、ダッシュボードの枠に入る。書き換えた内容は下書きとして保存し、保存しただけでは公開されている文書は変えない。公開すると下書きを GitHub の PR にしてマージキューへ載せ、マージされてデプロイされたときに公開されている文書が変わる。
...
- 操作のボタンは「下書きを保存」「保存した下書きを公開」「下書きを捨てる」「ページに戻る」。「下書きを捨てる」は下書きがあるときだけ出し、確認を経てから捨てる
- 「保存した下書きを公開」は、下書きがあり、公開の設定がある環境（本番）で「変更できる」権限の人にだけ出す。押すと、保存済みの下書きから PR を作る。保存していない編集は含めない
- 公開すると、PR を作ったことと、マージされてデプロイされるとページに反映されて下書きが消えることを示し、PR を開くボタンを置く。同じ内容のまま押し直しても PR は増やさず、同じ PR を返す
- 画像は公開のときに文書と同じコミットへ入れ、文書からは相対パスで参照する
- 公開されている文書が下書きを始めた後に更新されていたときは、公開せずに、下書きの内容を控えて最新の文書に反映し直すよう促す
