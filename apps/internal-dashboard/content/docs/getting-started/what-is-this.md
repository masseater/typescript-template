...
- **セッションリプレイは統合しない**: 利用者の操作を DOM の変化として録画し、trace id でトレースと結びつけて再生する機能です。録画は 1 セッションごとに DOM の全変化を持つため、保存と検索の費用がログやスパンより桁で大きく、事業の規模が決まる前に費用だけが先に立ちます。後から入れるときに要る OTLP の宛先の切り替えと `traceparent` の伝搬は既にあり、足りないのは録画のセッション識別子を要求へ載せる部分だけです。エラーの再現に利用者の操作の再現が要るようになったときにだけ検討し、その場合もエラーが起きたセッションだけを録画する構成から始めます。
