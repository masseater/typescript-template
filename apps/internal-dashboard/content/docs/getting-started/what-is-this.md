...
- **セッションリプレイ**: 利用者の操作を DOM の変化として録画し、trace id でトレースと結びつけて再生する機能。
  ...
  - OSS の例: rrweb（記録と再生のライブラリのみ）、OpenReplay（リプレイ専用）、ClickStack / HyperDX（ログ・トレースと同じ基盤）、PostHog（製品分析と同居）
  - SaaS の例: PostHog Cloud、Sentry、Microsoft Clarity、LogRocket、FullStory、Datadog RUM
- **行動分析**: ヒートマップ、ファネル、リテンションなど、利用者の操作を集計して製品の改善に使う機能。
  ...
  - OSS の例: PostHog（ファネルやリテンションまで）、Matomo、Umami、Plausible（ページ単位の集計）
  - SaaS の例: PostHog Cloud、Mixpanel、Amplitude、Hotjar（ヒートマップ中心）、Google Analytics
