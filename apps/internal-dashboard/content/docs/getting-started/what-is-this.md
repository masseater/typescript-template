...
- **セッションリプレイ**: 利用者の操作を DOM の変化として録画し、trace id でトレースと結びつけて再生する機能です。録画は 1 セッションごとに DOM の全変化を持つため、保存と検索の費用がログやスパンより 1 桁から 2 桁大きく、事業の規模が決まる前に費用だけが先に立ちます。SaaS の無料枠に載せても、録画数の上限がそのまま観測できる範囲の上限になります。後から入れるときに要る OTLP の宛先の切り替えと `traceparent` の伝搬は既にあり、足りないのは録画のセッション識別子を要求へ載せる部分だけです。エラーの再現に利用者の操作の再現が要るようになったときに、エラーが起きたセッションだけを録画する設定から入れます。
