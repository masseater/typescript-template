...
- **利用者アプリ**: 会員が登録してプロフィールを作り、ほかの会員を知ってやり取りするアプリ。
  ...
  - リアルタイムの受信: Durable Object が会員ごとに通知とフィードを持ち、WebSocket で届ける。
...
- **内部の呼び出し**: インターネットから届かない core Worker を、Service Binding 経由の RPC で呼ぶ。中身はまだ疎通の確認だけ。
...
- **インフラの宣言**: Workers、D1、KV、R2、Queues、Workflows、Durable Objects、Workers AI、メール送信などをコードで宣言し、管理画面での手作業を無くす。
