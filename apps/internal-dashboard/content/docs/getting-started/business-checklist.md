...
テンプレートはコードと運用の仕組みを持ちますが、計測、集客、請求、法令、信頼のような事業の決めごとは持ちません。ここにあるのは、詳しい人がいないと存在に気づかず、後から困る項目です。[場面ごとにやること](/checklists) の場面に合わせて並べ、各項目に何が起きるかと、テンプレートにすでにあるものを書きます。
