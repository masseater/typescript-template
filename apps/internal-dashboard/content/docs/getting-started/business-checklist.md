---
title: ビジネス面のチェックリスト
description: コードの外側で、知らないと後から困る決めごとを決める時期ごとに並べる
---

テンプレートはコードと運用の仕組みを持ちますが、計測、集客、請求、法令、信頼のような事業の決めごとは持ちません。ここにあるのは、詳しい人がいないと存在に気づかず、後から困る項目です。決める時期ごとに並べ、各項目に何が起きるかと、テンプレートにすでにあるものを書きます。
