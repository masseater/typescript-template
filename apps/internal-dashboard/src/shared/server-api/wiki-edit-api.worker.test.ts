...
function editApp(wikiAuth: Auth["Service"], publisher: Layer.Layer<WikiPublisher>) {
  ...
  const api = apiRoutes(runtime, { log: recordingSink().sink, service: APPLICATION.internalDashboard });
...
const signedInEditor = Effect.fn("signedInEditor")(function* signedInEditor(
  ...
  return {
    app: editApp((yield* AuthApps)[APPLICATION.internalDashboard], publisher),
