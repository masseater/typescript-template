...
管理者アプリには新規登録の入口を置かない。管理者のアカウントは、「管理者を追加できる」管理者の招待を受けて、招待を受けるページ（`/invite/{token}`）で作る。利用者アプリのアカウントとは別のもので、利用者アプリの資格情報ではログインできない。
...
```mermaid
flowchart TD
  choose["1. ログイン方法を選ぶ"] -- パスキー --> done["redirect のページ。無ければ /members"]
