...
パス: `/interview`。ログインした利用者だけが開ける。ログインしていないときは `/login?redirect=/interview` へ移る。
...
| 1 日に使える回数を使い切った | 今日はこれ以上続けられないことを会話の末尾に出し、それまでのシートは保つ |
...
```mermaid
flowchart TD
  open["/interview を開く"] --> signedIn{"ログインしている"}
  signedIn -- いいえ --> login["/login?redirect=/interview"]
  login -- ログインに成功 --> open
  signedIn -- はい --> asking["質問"]
