...
```bash
vp run --filter @template/observe observe request --app http://127.0.0.1:3001/ --request-id <x-request-id の値>
vp run --filter @template/observe observe logs --app http://127.0.0.1:3001/ --level error
vp run --filter @template/observe verify --app http://127.0.0.1:3001/
```
...
```bash
vp run --filter @template/observe symbolicate --app user --release <release> "/assets/index-abc.js:1:234"
```
