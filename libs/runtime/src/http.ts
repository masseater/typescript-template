...
const elysiaServer = (
  ...
  const handle = (runtimeContext: ElysiaContext): Promise<Response> => {
    return app.fetch(runtimeContext.request);
  };
  const handleHead = async (runtimeContext: ElysiaContext): Promise<Response> => {
    const { headers: asked, url } = runtimeContext.request;
    const httpResponse = await app.fetch(new Request(url, { headers: asked, method: "GET" }));
    const streamsEvents =
      httpResponse.headers.get("content-type")?.startsWith(eventStreamType) === true;
    if (streamsEvents) {
      void httpResponse.body?.cancel();
    }
    const requestBody = streamsEvents ? undefined : await httpResponse.arrayBuffer();
    const headers = new Headers([
      ...httpResponse.headers,
      ...(requestBody === undefined
        ? []
        : ([["content-length", String(requestBody.byteLength)]] as const)),
    ]);
...
const apiRoutes = <Requirements>(
  ...
  const openStream = <Value, Encoded extends ServerSentEvent, Failures extends Tagged>(
    ...
    return (
      ...
      ): Stream.Stream<FailedEvent, never, never> =>
  ...
  const settle = async <Value>(
    ...
    const exit = await runtime.runPromiseExit(settled.program(runtimeContext.request));
    return Exit.isSuccess(exit) ? exit.value : Effect.runPromise(settled.unavailable(exit.cause));
  };
  const raw = <Failures extends Tagged>(
    routeHandler: Handler<Response, Failures, Requirements>,
    failures: FailureTable<Exclude<Failures, CommonFailure>>,
  ): ElysiaHandler => {
    return async (runtimeContext): Promise<Response> =>
  ...
  const route = <Value, Encoded, Failures extends Tagged>(
    ...
    return (
      ...
      return async (runtimeContext): Promise<Encoded | Failed> =>
        settle(runtimeContext, {
          program: respondValue(httpResponse)(routeHandler, failures),
  ...
  const logEvents = <Value, Encoded extends ServerSentEvent, Failures extends Tagged>(
    ...
    return (
      routeHandler: Handler<Stream.Stream<Value, never, Requirements>, Failures, Requirements>,
      failures: FailureTable<Exclude<Failures, CommonFailure>>,
    ): ((runtimeContext: ElysiaStreamContext) => Promise<EventStream<Encoded> | Failed>) => {
      const open = openStream(logEvent)(routeHandler, failures);
      return async (runtimeContext): Promise<EventStream<Encoded> | Failed> => {
        const opened = await settle(runtimeContext, {
          program: open,
          unavailable: (cause) => unavailableStatus(cause, reporting),
        });
