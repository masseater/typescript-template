...
const patterns = {
  attributeKey: /"key":"([^"]+)"/gu,
  body: /"body":\{"stringValue":"([^"]+)"\}/gu,
  severityText: /"severityText":"([^"]+)"/gu,
  traceId: /"traceId":"([0-9a-f]{32})"/gu,
} as const;
...
const captured = (exportedText: string, named: keyof typeof patterns): readonly string[] =>
  distinct(
    Array.from(exportedText.matchAll(new RegExp(patterns[named], "gu")), (match) => match[1] ?? ""),
  );

const traceparentPattern = /^00-([0-9a-f]{32})-[0-9a-f]{16}-01$/u;
...
const summarized = (summary: {
  ...
  const requestTraceId = traceparentPattern.exec(summary.traceparent)?.[1] ?? "";
  return {
