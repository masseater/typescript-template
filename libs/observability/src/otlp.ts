...
const reportRejection = (
  ...
  answered.status >= httpStatus.badRequest
    ? Effect.logWarning("otlp.export_failed").pipe(annotateLogs({ "otlp.status": answered.status }))
    : Effect.void;

const reportFailure = (
  refused: Readonly<Pick<HttpClientError.HttpClientError, "_tag">>,
): Effect.Effect<void> =>
  Effect.logWarning("otlp.export_failed").pipe(annotateLogs({ "otlp.error": refused._tag }));
