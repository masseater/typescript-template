...
import { bootstrapStatement, readBootstrappedAdmin } from "./bootstrap-statement.ts";
...
const BootstrappedRow = Schema.Array(Schema.Unknown);
...
const bootstrapDatabase = <Result>(
  ...
  return Effect.gen(function* bootstrap() {
    ...
    yield* readBootstrappedAdmin(row).pipe(
      Effect.mapError(() => new RemoteFailure({ code: "REMOTE_RESPONSE_INVALID" })),
    );
