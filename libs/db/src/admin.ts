...
import { AUDIT_ACTION, AUDIT_CHANNEL, user, type AuditChannel } from "./schema.ts";
...
export const getMember = Effect.fn("getMember")(function* getMember(
  ...
  const [member] = yield* query((database) =>
    database
      .select(memberColumns)
      .from(user)
      .where(and(eq(user.id, memberId), eq(user.role, ROLE.member), liveAdmin(database, sessionId)))
      .limit(1),
  );
  if (!member) {
