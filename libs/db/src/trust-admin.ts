import { MODERATION_KIND, REPORT_STATUS } from "@repo/config";
...
const readReport = Effect.fn("readReport")(function* readReport(
  ...
  const target =
    row.targetMemberId === null
      ...
      : (yield* query((database) =>
          database
            .select({ email: user.email, name: user.name, suspended: user.suspended })
            .from(user)
  ...
  return {
    ...
    targetSuspended: target?.suspended ?? false,
  };
});

function suspendAudit(change: {
  readonly action: (typeof AUDIT_ACTION)["memberSuspended" | "memberUnsuspended"];
  readonly actorId: string;
  readonly targetId: string;
}) {
  return auditWhen(change, sql`SELECT 1 FROM ${user} WHERE ${user.id} = ${change.targetId}`);
}
...
const suspendTarget = Effect.fn("suspendTarget")(function* suspendTarget(
  ...
  const [, updated] = yield* query(async (database) => {
    const audit = database.run(suspendAudit(change));
    const suspension = database
      .update(user)
      .set({ suspended, updatedAt: now })
      .where(eq(user.id, targetId))
      .returning({ id: user.id });
    const action = database.insert(moderationAction).values({
      actorId: actor.user.id,
      createdAt: now,
    ...
    const filed = database
      ...
      .where(eq(memberReport.id, reportId));
    return database.batch([audit, suspension, action, filed] as const);
  });
  if (updated.length === 0) {
    return yield* new TrustTargetUnavailable();
  }
