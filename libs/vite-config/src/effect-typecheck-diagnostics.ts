...
const withoutPnpmPackagePath = (sourceText: string): string =>
  sourceText.replaceAll(/node_modules\/\.pnpm\/[^/]+/gu, `node_modules/.pnpm/${pnpmPackageMarker}`);

const portableDiagnostic = (diagnostic: Diagnostic, repositoryRootPath: string): Diagnostic => ({
  file: withoutPnpmPackagePath(withoutCheckoutPath(diagnostic.file, repositoryRootPath)),
  code: diagnostic.code,
  message: withoutPnpmPackagePath(withoutCheckoutPath(diagnostic.message, repositoryRootPath)),
});
