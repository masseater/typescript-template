...
import { parseBaseline, portableBaseline, serializeBaseline } from "./effect-typecheck-baseline.ts";
...
const compareBaseline = (
  ...
  const verdict = evaluateTypecheck({
    ...
    snapshotted: Object.values(loadBaseline(gate.asked).workspaces).flat(),
  });
  if (!verdict.ok) {
